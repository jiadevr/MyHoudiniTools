...
class ProjectManager(QtWidgets.QMainWindow):
    ...
    def __init__(self) -> None:
        super().__init__()
        ui_path = hou.text.expandString("$MYLIB/ui/project_manager.ui").replace(
            os.sep, "/"
        )
        ...
        self.config_file_path = hou.text.expandString(
            "$MYLIB/configs/ProjectConfig.json"
        ).replace(os.sep, "/")
        ...
        if avaliable_proj:
            self.lw_project.setCurrentItem(
                self.lw_project.findItems(
                    avaliable_proj, QtCore.Qt.MatchFlag.MatchExactly
                )[0]
            )
    ...
    def _InitialUI_(self):
        ...
        """
        self.lw_project: QtWidgets.QListWidget = self.ui.findChild(
            QtWidgets.QListWidget, "lw_projects"
        )

        self.project_detail: QtWidgets.QPushButton = self.ui.findChild(
            QtWidgets.QPushButton, "bt_proj_details"
        )
        self.project_enable: QtWidgets.QPushButton = self.ui.findChild(
            QtWidgets.QPushButton, "bt_proj_enable"
        )
        self.project_disable: QtWidgets.QPushButton = self.ui.findChild(
            QtWidgets.QPushButton, "bt_proj_disable"
        )
        self.delete_project: QtWidgets.QPushButton = self.ui.findChild(
            QtWidgets.QPushButton, "bt_proj_delete"
        )

        self.lw_seq: QtWidgets.QListWidget = self.ui.findChild(
            QtWidgets.QListWidget, "lw_seq"
        )
        self.create_scene: QtWidgets.QPushButton = self.ui.findChild(
            QtWidgets.QPushButton, "bt_create_scene"
        )
        self.delete_scene: QtWidgets.QPushButton = self.ui.findChild(
            QtWidgets.QPushButton, "bt_delete_scene"
        )

        self.lw_files: QtWidgets.QListWidget = self.ui.findChild(
            QtWidgets.QListWidget, "lw_files"
        )
        self.open_file: QtWidgets.QPushButton = self.ui.findChild(
            QtWidgets.QPushButton, "bt_open"
        )
        self.save_file: QtWidgets.QPushButton = self.ui.findChild(
            QtWidgets.QPushButton, "bt_save"
        )

        self.state_line: QtWidgets.QLineEdit = self.ui.findChild(
    ...
    def _RaiseAMessage_(
        self,
        in_message,
        in_severity=hou.severityType.Message,
        bshow_dialog: bool = False,
    ):
        """
        ...
        if bshow_dialog or (
            in_severity == hou.severityType.Error
            or in_severity == hou.severityType.Warning
            or in_severity == hou.severityType.Fatal
        ):
    ...
    def GetSelectedScene(self):
        ...
        if target_project_name == None:
            self._RaiseAMessage_(
                "Please Select A Project First", hou.severityType.Error
            )
            return None, None

        project_seq_dir: str = os.path.join(
            target_project_info["projectPath"], "seq"
        ).replace(os.sep, "/")

        if not os.path.exists(project_seq_dir):
            self._RaiseAMessage_(
                f"Give Project{target_project_name} 's Paths {project_seq_dir} Doesn't Existed",
        ...
        selected_scene_dir = os.path.join(project_seq_dir, selected_scene).replace(
            os.path.sep, "/"
        )

        if not os.path.exists(selected_scene_dir):
            self._RaiseAMessage_(
                f"Target Path {selected_scene_dir} Doesn't Existed",
                hou.severityType.Error,
            )
            return None, None
    ...
    def ShowProjectDetails(self):
        ...
        if not self.lw_project.selectedItems:
            self._RaiseAMessage_(
                "Please Select A Project In Project List",
                in_severity=hou.severityType.Error,
            )
        ...
        if project_data:
            self._RaiseAMessage_(
                f"Project Detail for {'target_project'},\n"
            ...
            self._RaiseAMessage_(
                f"Find Null Project Named {target_project}", hou.severityType.Error
            )
    ...
    def ToggleProjectEnable(self, status=True):
        ...
        if not self.project_data:
            self._RaiseAMessage_(
                "Fail To Change Enable State Config, Empty project_data"
            )
            return
        if status == True:
            # 关闭其他项目启用状态
    ...
    def DeleteProject(self):
        ...
        user_choice = hou.ui.displayMessage(
            f"This Action Will Detele Project And All Project Files!!!",
            buttons=("Yes", "Cancel"),
        ...
        if os.path.exists(project_dir):
            try:
                ...
                self._RaiseAMessage_(
                    f"Failed To Remove Project {target_project} Files: Reason{Error}",
                    hou.severityType.Error,
                )
    ...
    def RefreshSceneList(self):
        ...
        if not os.path.exists(seq_dir):
            self._RaiseAMessage_(
                f"Find Null Sequene File In Project Named {target_project}"
            )
            return
    ...
    def CreateScene(self):
        ...
        if not os.path.exists(project_dir):
            self._RaiseAMessage_(
                f"Give Project{target_project} 's Paths {project_dir} Doesn't Existed",
                hou.severityType.Error,
            )
            return
        ...
        user_inputs_tuple = hou.ui.readMultiInput(
            message="Create New Scene",
            input_labels=lable_tint,
            buttons=("OK", "Cancel"),
            initial_contents=user_inputs,
        )
        ...
        if os.path.exists(target_dir):
            user_override = hou.ui.displayMessage(
                "Given Path Is Already Existed,Do You Want To Override All Files?",
                buttons=("OK", "Cancel"),
            )
            if user_override == 1:
                self._RaiseAMessage_(
                    "Creating New Scene was Cancelled,Reason : Duplicated Path",
                    hou.severityType.Error,
                )
                return
        else:
    ...
    def DeleteScene(self):
        ...
        if (
            ...
            or selected_scene_dir == None
            or (not os.path.exists(selected_scene_dir))
        ):
            self._RaiseAMessage_(
                f"Give Project{selected_scene_name} 's Paths {selected_scene_dir} Doesn't Existed",
                hou.severityType.Error,
            )
            return

        user_input = hou.ui.displayMessage(
            f"Would You Like Delete All Files In {selected_scene_name}",
            buttons=("OK", "Cancel"),
            severity=hou.severityType.Warning,
        )
    ...
    def OpenFile(self):
        ...
        for dirpath, dirnames, filenames in os.walk(selected_scene_dir):
            for file_name in filenames:
                ...
                if file_name == target_hip:
                    hip_path = os.path.join(dirpath, file_name).replace(
                        os.path.sep, "/"
                    )
        ...
        if not os.path.exists(hip_path):
            self._RaiseAMessage_(
                f"Fail to Open {target_hip}, Path {hip_path} Doesn't Exist!",
                hou.severityType.Error,
            )
            return
        try:
    ...
    def SaveFile(self):
        ...
        if selected_scene_name == None or selected_scene_dir == None:
            self._RaiseAMessage_(
                "Please Select A Seq In Project", hou.severityType.Error
            )
            return

        self._RaiseAMessage_(
            f"Ready To Save {selected_proj_name}-{selected_scene_name}"
        )
        # 创建实例
        if not self.save_sub_widget:
            self.save_sub_widget = SaveToolWindow(
                selected_scene_dir, selected_proj_name, selected_scene_name
            )
