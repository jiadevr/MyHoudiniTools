...
class SaveToolWindow(QtWidgets.QWidget):
    ...
    def __init__(
        self, in_scene_dir=None, in_project_name=None, in_seq_name=None
    ) -> None:
        super().__init__()
        # 基础窗口设置
    ...
    def _init_tints_(self):
        self.project_label.setText(
            f"Target Project:{self.project_name},Target Seq:{self.seq_name}"
        )
    ...
    def RefreshSavePath(self):
        ...
        user_input_file_name = (
            self.file_name_input.text().replace(" ", "_").strip() or "Unnamed"
        )
        ...
        self.save_path = os.path.join(self.scene_dir, self.save_path).replace(
            os.sep, "/"
        )
    ...
    def GetCurrentVersion(self, in_name_prefix, in_extension="hip") -> int:
        if not self.scene_dir == None and os.path.exists(self.scene_dir):
            ...
            hou.ui.displayMessage(
                "Empty Path ,Get Scene Dir Failed,Please Check If The Path Exist"
            )
        return 1
    ...
    def SaveProj(self):
        ...
        try:
            ...
            hou.ui.displayMessage(
                f"Fail to Save hip File,Reason: OS Error,Target Path:{self.save_path}"
            )
