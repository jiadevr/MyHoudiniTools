...
class ProjectCreator(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        # 初始化对象引用
        self.dir_selector: QtWidgets.QPushButton = self.ui.findChild(
            QtWidgets.QPushButton, "bt_directory"
        )
        self.project_name: QtWidgets.QLineEdit = self.ui.findChild(
            QtWidgets.QLineEdit, "le_proj_name"
        )
        self.project_code: QtWidgets.QLineEdit = self.ui.findChild(
            QtWidgets.QLineEdit, "le_proj"
        )
        self.project_fps: QtWidgets.QLineEdit = self.ui.findChild(
            QtWidgets.QLineEdit, "le_fps"
        )
        self.folders: QtWidgets.QPlainTextEdit = self.ui.findChild(
            QtWidgets.QPlainTextEdit, "qpt_folders"
        )
        self.create_project: QtWidgets.QPushButton = self.ui.findChild(
            QtWidgets.QPushButton, "bt_create_project"
        )
    ...
    def checkButtonState(self):
        ...
        if (
            ...
            and self.project_code.text
            and self.project_fps.text
            and self.folders.getPaintContext
        ):
    ...
    def createProjectFiles(self):
        ...
        user_project_path = os.path.join(self.project_dir, user_project_name).replace(
            os.sep, "/"
        )
        ...
        for single_item in data:
            ...
            if item_name == user_project_name or item_code == user_project_code:
                hou.ui.displayMessage(
                    f"There already a record with same name or code:\n\n"
