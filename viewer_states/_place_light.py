...
HUD_TEMPLATE = {
    "title": "High Light Placer", "desc": "tool", "icon": "LOP_light",
    "rows": [
        {"id": "place", "label": "Brush Shape", "key": "LMB"},
        {"id": "modedev", "type": "divider"},

        {"id": "distance", "label": "Distance", "key": "mousewheel","value":"0.0"},
    ]
}
...
class State(object):
    ...
    def _get_light_position_by_reflection_(
        self,
        in_current_viewport: hou.GeometryViewport,
        in_mouse_x: int,
        in_mouse_y: int,
    ) -> bool:
        ...
        view_dir, mouse_position = in_current_viewport.mapToWorld(
        ...
        primitive_number, hit_position_local, hit_normal_local, uvw_coordinates = (
            su.sopGeometryIntersection(geo_object, mouse_position_local, view_dir_local)
        )
        # 转换回世界坐标
    ...
    def onMouseEvent(self, kwargs):
        ...
        self.scene_viewer.setPromptMessage(
            "Chick with LMB or Press and Drag LMB to Place Light"
        )
        if (
            mouse_operation == hou.uiEventReason.Picked
            or mouse_operation == hou.uiEventReason.Active
        ):
            if self.light:
                # 获取当前视口
                ...
                bsuccess: bool = self._get_light_position_by_reflection_(
                    current_viewport, int(dev.mouseX()), int(dev.mouseY())
                )
    ...
    def onMouseWheelEvent(self, kwargs):
        ...
        if self.light:
            ...
            self.light_distance = max(0.0, self.light_distance + delta_value)
            self.light_position = self.hit_location + (
                self.light_dir * self.light_distance
            )
            light_matrix = hou.hmath.buildRotateZToAxis(self.light_dir)
            light_matrix *= hou.hmath.buildTranslate(self.light_position)
            self.light.setWorldTransform(light_matrix)
            ...
            updates={"distance":f"{self.light_distance:0.2f}"}
    ...
    def onMenuAction(self, kwargs):
        ...
        # 直接使用返回值加_前缀索引目标函数
        target_method=getattr(self,"_"+menu_item)
    ...
    def onGenerate(self, kwargs):
        ...
        print("Begin State Undo")
        # 注意不要用_hudInfo，会报错
        self.scene_viewer.hudInfo(template=HUD_TEMPLATE)

def _create_context_menu_():
    context_menu = hou.ViewerStateMenu("change_light_type", "Change Light Type")
    context_menu.addActionItem("grid", "Grid")
    context_menu.addActionItem("disk", "Disk")
    context_menu.addActionItem("sun", "Sun")
    return context_menu
...
def createViewerStateTemplate():
    ...
    # 上下文菜单
    template.bindMenu(_create_context_menu_())
    return template
